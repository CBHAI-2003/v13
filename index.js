  ...
  try {
    ...
    if (m === 'GET' && p === '/') {
      return res.json({
        ...
        buy_tokens: process.env.LEMON_SQUEEZY_URL || 'Configure your Lemon Squeezy store URL',
        documentation: {
          'OpenAI Compatible': {
            url: 'https://v13-gray.vercel.app/v1/chat/completions',
            auth: 'Bearer YOUR_API_KEY',
            models: Object.keys(MODELS).filter(k => MODELS[k].key),
          },
          'Simple API': {
            register: 'POST /register (get free $10)',
            chat: 'POST /chat (Authorization: Bearer KEY)',
            check_balance: 'GET /balance (Authorization: Bearer KEY)',
            redeem_code: 'POST /redeem (body: {"code":"..."})',
          },
        },
      });
    }
    ...
    if (m === 'POST' && (p === '/v1/chat/completions' || p === '/chat')) {
      const u = getUser();
      if (!u) return res.status(401).json({ error: 'Invalid API key. Register at POST /register' });
      ...
      const r = await fetch(MODELS[key].url, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${MODELS[key].key}` },
        body: JSON.stringify({ model: MODELS[key].model, messages }),
      });
      ...
      u.balance -= cost;
      u.spent += cost;
      d._balance = { remaining: u.balance, spent: u.spent };
      return res.json(d);
    }
    ...
    if (m === 'POST' && p === '/register') {
      const key = genKey();
      DB.users.push({ id: crypto.randomUUID(), api_key: key, balance: 1000, spent: 0 });
      return res.json({ api_key: key, balance: 1000, chat_endpoint: 'https://v13-gray.vercel.app/v1/chat/completions' });
    }
    ...
    if (m === 'POST' && p === '/redeem') {
      ...
      const t = DB.tokens.find(t => t.code === code && !t.used);
    ...
    // ══════ Webhook ══════
    if (m === 'POST' && p === '/webhook') {
      const total = req.body?.data?.attributes?.total || 0;
      const amt = Math.max(Math.round(total * 100), 1000);
      const c = genCode();
      DB.tokens.push({ id: crypto.randomUUID(), value: amt, code: c, used: false });
      const email = req.body?.data?.attributes?.user_email;
      if (email) {
        const u = DB.users.find(x => x.email === email);
        if (u) { u.balance += amt; DB.tokens.find(t => t.code === c).used = true; }
      }
